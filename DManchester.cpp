/*
This code is based on the Atmel Corporation DifferencialManchester
...
"DifferencialManchester coding states that there will always be a transition of the message signal
...
We use Timing Based DifferencialManchester Decode.
...
void DifferencialManchester::setTxPin(uint8_t pinA, uint8_t pinB)
{
  TxPin = pinA; // user sets the digital pin as output
  TxPin2 = pinB;
...
void DifferencialManchester::setRxPin(uint8_t pin)
{
  ::RxPin = pin; // user sets the digital pin as output
  pinMode(::RxPin, INPUT); 
}

void DifferencialManchester::workAround1MhzTinyCore(uint8_t a)
{
  applyWorkAround1Mhz = a;
}

void DifferencialManchester::setupTransmitPwr(uint8_t pinA, uint8_t pinB, uint8_t SF)
{
    setTxPin(pinA,pinB);    
    setupTransmit(pinA,SF);
}

void DifferencialManchester::setupTransmit(uint8_t pin, uint8_t SF)
{
  setTxPin(pin);
  speedFactor = SF;
...
void DifferencialManchester::setupReceive(uint8_t pin, uint8_t SF)
{
  setRxPin(pin);
...
void DifferencialManchester::setup(uint8_t Tpin, uint8_t Rpin, uint8_t SF)
{
...
void DifferencialManchester::transmit(uint8_t data)
{
...
*/
void DifferencialManchester::transmitArray(uint8_t numBytes, uint8_t *data)
{
...
uint8_t DifferencialManchester::decodeMessage(uint16_t m, uint8_t &id, uint8_t &data)
{
  //extract components
  data = (m & 0xFF);
  id = (m >> 12);
...
uint16_t DifferencialManchester::encodeMessage(uint8_t id, uint8_t data)
{
...
void DifferencialManchester::beginReceiveArray(uint8_t maxBytes, uint8_t *data)
{
  ::MANRX_BeginReceiveBytes(maxBytes, data);
}

void DifferencialManchester::beginReceive(void)
{
  ::MANRX_BeginReceive();
}


uint8_t DifferencialManchester::receiveComplete(void)
{
  return ::MANRX_ReceiveComplete();
}


uint8_t DifferencialManchester::getMessage(void)
{
  return ::MANRX_GetMessage();
}


void DifferencialManchester::stopReceive(void)
{
  ::MANRX_StopReceive();
}
...
void MANRX_SetupReceive(uint8_t speedFactor)
{
  ...
  //setup timers depending on the microcontroller used
    ...
    */
    ...
    #else
    #error "DifferencialManchester library only supports 1mhz, 8mhz, 16mhz, 16.5Mhz clock speeds on ATtiny85 chip"
    #endif
    ...
    */
      ...
      OCR1A = (64 >> speedFactor) - 1; 
    #else
    #error "DifferencialManchester library only supports 1mhz, 8mhz clock speeds on ATtiny2313 chip"
    #endif
    ...
    */
    ...
    #else
    #error "DifferencialManchester library only supports 1mhz, 8mhz, 16mhz on ATtiny84"
    #endif
    ...
    */
    ...
    #else
    #error "DifferencialManchester library only supports 1mhz, 8mhz, 16mhz on ATMega32U4"
    #endif
    ...
    */
    ...
    #else
    #error "DifferencialManchester library only supports 1Mhz, 8mhz, 16mhz on ATMega8"
    #endif
    ...
    */
    ...
    TCCR2A = _BV(WGM21); // reset counter on match
      ...
      OCR2A = (128 >> speedFactor) - 1; 
    #else
    #error "DifferencialManchester library only supports 8mhz, 16mhz on ATMega328"
    #endif
