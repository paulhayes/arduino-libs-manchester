...
This code is based on the Atmel Corporation Differencial
...
"Differencial coding states that there will always be a transition of the message signal
...
We use Timing Based Differencial Decode.
...
#ifndef DMANCHESTER_h
#define DMANCHESTER_h
...
class DifferencialManchester
{
  public:
    DifferencialManchester(); //the constructor
    void setTxPin(uint8_t pin); //set the arduino digital pin for transmit. 
    void setTxPin(uint8_t pinA, uint8_t pinB);
    ...
    void setupTransmitPwr(uint8_t pinA, uint8_t pinB, uint8_t SF = MAN_1200);
  ...
  private:
    ...
    void sendOne(void);
    uint8_t TxPin;
    //inverse ( for powerline signals )
    uint8_t TxPin2 = 254;
    uint8_t lastWrite = 0;
    uint8_t applyWorkAround1Mhz;
};//end of class Differencial
...
extern DifferencialManchester dman;
